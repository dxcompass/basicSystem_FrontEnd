...
function expressionChange() {
    // 始める-結果
    isShow.value = false;
    // ルール配列を取得します[02番、1ポイント、2時間、3日、4月、5週、6年]
    let ruleArr = props.ex.split(' ');
    // ループに入る回数を記録するために使用されます
    let nums = 0;
    // のために暂時間存符号時間间规则结果的配列
    let resultArr = [];
    // 得る当前時間间精确至[年、月、日、時間、ポイント、2番]
    let nTime = new Date();
    ...
    // 電流の場合“日”配列内の現存在するの値ではありません
    if (nDay !== DDate[DIdx]) {
        resetHour();
    }
    // 電流の場合“時間”配列内の現存在するの値ではありません
    if (nHour !== hDate[hIdx]) {
        resetMin();
    }
    // 電流の場合“ポイント”配列内の現存在するの値ではありません
    if (nMin !== mDate[mIdx]) {
        resetSecond();
    }
    // サイクル年アレイ
        ...
        goMonth: for (let Mi = MIdx; Mi < MDate.length; Mi++) {
            ...
            goDay: for (let Di = DIdx; Di < DDate.length; Di++) {
                ...
                if (dayRule.value === 'lastDay') {
                    // それが法律の日付でない場合は、前の日付を法的日付、つまり月末の最終日に調整する必要があります。
    ...
    if (resultArr.length === 0) {
        ...
        if (resultArr.length !== 5) {
            resultList.value.push('最近100年以上' + resultArr.length + '精査！')
        }
    }
    // 完全に計算します-結果を示す
    isShow.value = true;
}
// のために计算某位数字存在する配列真ん真ん中的索引
